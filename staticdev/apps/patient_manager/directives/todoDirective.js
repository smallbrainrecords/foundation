            ...
            scope.$watch('todos_ready', function (newVal, oldVal) {
                if (newVal) {
                    ...
                    if (scope.todos_ready) {
                        ...
                        scope.grouped_todos = scope.$eval(attr.grouped);
                        ...
                        scope.sortableOptions = {
                            ...
                            receive: function (event, ui) {
                                // Get dragged item model.
                                // Dragged able item: Group | Todo
                                var draggedItem = ui.item.sortable.source.context;
                                var draggedData = $(draggedItem).data();

                                // Get detail about the place where item being dropped
                                // Dropped able item: Group | Todo
                                var dropTargetDOM = ui.item.sortable.droptarget;
                                var droppedData = $(dropTargetDOM).data();

                                if (draggedData.isGroup && droppedData.isGroup) {
                                    // Drag group into other group
                                    // Cancel the event
                                    ui.item.sortable.cancel();
                                } else if (draggedData.isGroup && !droppedData.isGroup) {
                                    // Drag group into a todo
                                    ui.item.sortable.cancel();
                                } else if (!draggedData.isGroup && droppedData.isGroup) {
                                    // Continue section add this dragged model to group
                                    scope.addTodoToGroup(draggedData.itemId, droppedData.itemId);
                                } else if (!draggedData.isGroup && !droppedData.isGroup) {
                                    // How to get new group order
                                    ui.item.sortable.cancel();
                                    scope.createTodoGroup(draggedData.itemId, droppedData.itemId, ui.item.sortable.index);
                                }
                            },
                    ...
                    scope.update_todo_status = function (todo) {
                        ...
                        patientService.updateTodoStatus(todo).then(function (data) {
                            if (data['success'] == true) {
                                ...
                                // Update data tree with newly grouped
                    ...
                    /**
                     * Update root item after save data to root model
                     * @param data
                     */
                    scope.updateTodoTree = function (data) {
                        scope.grouped_todos = null;
                        $timeout(function () {
                            // View is not updated
                            scope.grouped_todos = {
                                name: "rootItem",
                    ...
                    /**
                     * Create new todoGroup from two other todo
                     * @param todo_id_1
                     * @param todo_id_2
                     */
                    scope.createTodoGroup = function (todo_id_1, todo_id_2) {
                        var form = {};
                        form.name = "Folder";
                        form.todo_1 = todo_id_1;
                        form.todo_2 = todo_id_2;
                        todoService.createToDoGroup(form, scope.patient_id).then(function (data) {
                            if (data['success'] == true) {
                                // Notify user
                                toaster.pop('success', 'Done', 'Success add new group');
                     ...
                     * @param group_id
                     */
                    scope.addTodoToGroup = function (todo_id, group_id) {
                        var form = {};
                        form.patient = scope.patient_id;
                        form.todo = todo_id;
                        form.group = group_id;
                        todoService.addToDoToGroup(form).then(function (data) {
                            if (data['success'] == true) {
                                toaster.pop('success', 'Done', 'Success add todo to group');
                                scope.updateTodoTree(data);
                     ...
                     * @param group_id
                     */
                    scope.removeTodoFromGroup = function (todo_id, group_id) {
                        var form = {};
                        form.patient = scope.patient_id;
                        form.todo = todo_id;
                        form.group = null;
                        todoService.removeToDoGroup(form).then(function (data) {
                            if (data['success'] == true) {
                                toaster.pop('success', 'Done', 'Success add todo to group');
                                scope.updateTodoTree(data);
                     ...
                     * @param group_id
                     */
                    scope.updateToDoGroup = function (group_id) {
                        var form = {};
                        form.name = "New folder name!!!!!";
                        form.order = _.random(1, 100);
                        form.patient_id = patient_id;
                        todoService.updateToDoGroup(form, group_id).then(function (data) {
                            if (data['success'] == true) {
                                toaster.pop('success', 'Done', 'Success add new group');
                                scope.updateTodoTree(data);
                     ...
                     * @params: groupId
                     */
                    scope.destroyTodoGroup = function (group_id) {
                        var form = {};
                        form.patient = scope.patient_id;
                        form.group = group_id;
                        todoService.deleteToDoGroup(form, group_id).then(function (data) {
                            if (data['success'] == true) {
                                toaster.pop('success', 'Done', 'Success add todo to group');
                                scope.updateTodoTree(data);

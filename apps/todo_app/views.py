...
def get_todo_activity(request, todo_id, last_id):
    ...
    resp = {
        "activities": TodoActivitySerializer(activities, many=True).data,
        "success": True,
    }
...
def update_order(request):
    ...
    if datas.has_key("tagged_user_id"):
        ...
        for id in id_todos:
            ...
            if not todos_order[key]:
                order = TaggedToDoOrder.objects.filter(user=user).aggregate(
                    Max("order")
                )
                if not order["order__max"]:
                    order = 1
...
def get_todo_info(request, todo_id):
    ...
    encounter_ids = EncounterTodoRecord.objects.filter(todo=todo_info).values_list(
        "encounter__id", flat=True
    )
    related_encounters = Encounter.objects.filter(id__in=encounter_ids)

    activities = TodoActivity.objects.filter(todo=todo_info)

    sharing_patients = SharingPatient.objects.filter(shared=todo_info.patient).order_by(
        "sharing__first_name", "sharing__last_name"
    )
    ...
    resp = {
        "success": True,
        "info": TodoSerializer(todo_info).data,
        "comments": ToDoCommentSerializer(comments, many=True).data,
        "attachments": attachment_todos_holder,
    ...
    if not name == "screening":
        ...
        if not label:
            label = Label.objects.create(
                name=name, css_class=css_class, author=request.user, is_all=is_all
            )
...
def add_todo_attachment(request, todo_id):
    ...
    add_todo_activity(
        attachment.todo, request.user, activity, comment=None, attachment=attachment
    )
    return ajax_response(resp)
...
def remove_todo_member(request, todo_id):
    ...
    todo = ToDo.objects.get(id=todo_id)
    # member_id = request.POST.get("id")
    user_profile_id = request.POST.get("profile[id]")
    member = UserProfile.objects.get(id=int(user_profile_id))
...
def get_labels(request, user_id):
    labels = Label.objects.filter(
        Q(is_all=True) | (Q(is_all=False) & Q(author_id=user_id))
    )
...
def get_user_todos(request, user_id):
    ...
    new_tagged_todo_count = (
        TaggedToDoOrder.objects.filter(user_id=user_id).filter(status=0).count()
    )

    tagged_todo_order = TaggedToDoOrder.objects.filter(user_id=user_id).order_by(
        "order", "todo__due_date"
    )
    tagged_todos = [t.todo for t in tagged_todo_order]
    serialized_data = TodoSerializer(tagged_todos, many=True).data
    for item in serialized_data:
        tagged_todo_instance = (
            TaggedToDoOrder.objects.filter(todo_id=item["id"])
    ...
    personal_todos = (
        ToDo.objects.filter(user_id=user_id)
        .filter(patient=None)
...
def add_staff_todo_list(request, user_id):
    ...
    todos = (
        ToDo.objects.filter(labels__id__in=label_ids).distinct().order_by("due_date")
    )
...
def get_user_label_lists(request, user_id):
    ...
    admin_labeled_todo_list = LabeledToDoList.objects.filter(
        user__profile__role="admin", private=0
    ).all()
    ...
    physician_labeled_todo_list = (
        LabeledToDoList.objects.filter(user_id__in=physician_ids)
        .filter(private=0)
        .all()
    )

    merged_list = (
        list(lists) + list(admin_labeled_todo_list) + list(physician_labeled_todo_list)
    )
    lists_holder = []
...
def staff_all_todos(request, user_id):
    ...
    todos = ToDo.objects.filter(
        accomplished=False, patient__id__in=patient_ids, due_date__lte=datetime.now()
    ).order_by("-due_date")
...
def batch_save_activities(request):
    ...
    for todo_id in todo_id_list:
        TodoActivity(
            todo_id=todo_id,
            author=request.user,
            activity="<b>{0} {1} - {2} </b> print this todo".format(

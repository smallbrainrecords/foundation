...
from emr.settings import PROBLEMS_PATH
...
def view_patient(request, user_id):
    ...
    import os
    import re
    context['problem_elements'] = [ re.search('problem_(?P<element>\w+)', f).group('element') for f in os.listdir(PROBLEMS_PATH) if not (f == 'problem_element_template.js' or f == 'problems.js')]

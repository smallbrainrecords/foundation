...
def home(request):
    ...
    return render(request, "project_admin/../../templates/admin-app.html", content)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            date_of_birth = dateutil.parser.parse(
                request.POST.get("date_of_birth"), ignoretz=True
            )
            ...
            user_profile.user.is_active = (
                request.POST.get("is_active").lower() == "true"
            )
                ...
                if new_user is not None:
                    ...
                    if actor_profile is not None:
                        if role == "patient":
                            if actor_profile.role == "physician":
                                patient_controller = PatientController(
                                    patient=new_user, physician=actor, author=True
                                )
                                patient_controller.save()

                            if (
                                actor_profile.role == "mid-level"
                                or actor_profile.role == "nurse"
                                or actor_profile.role == "secretary"
                            ):
                        ...
                        if (
                            ...
                            if actor_profile.role == "physician":
                                team = PhysicianTeam(member=new_user, physician=actor)
                                team.save()
...
def assign_physician_member(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                user = User.objects.get(id=user_id)
                user_profile = UserProfile.objects.get(user=user, role=member_type)
            except User.DoesNotExist:
            ...
            if user_profile is not None and physician_profile is not None:
                if member_type == "patient":
                    controller = PatientController(patient=user, physician=physician)
                    controller.save()
                else:
                    team_member = PhysicianTeam(member=user, physician=physician)
                    team_member.save()
                success = True
        ...
        if member_type in ["nurse", "secretary", "mid-level", "patient"]:
            ...
            if user is not None:
                ...
                unassigned_physicians = UserProfile.objects.filter(
                    role="physician"
                ).exclude(user__id__in=physician_ids)
                unassigned_physicians_list = UserProfileSerializer(
                    unassigned_physicians, many=True
                ).data

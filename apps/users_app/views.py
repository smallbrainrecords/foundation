...
def manage_patient(request, user_id):
    ...
    context["bleeding_risk"] = json.dumps(
        Medication.objects.filter(current=True)
        .filter(concept_id__in=MEDICATION_BLEEDING_RISK)
        .filter(patient=user)
        .exists()
    )
...
def get_patient_info(request, patient_id):
    ...
    if request.user.profile.role == "admin":
        ...
        for problem in problem_list:
            todo = ToDo.objects.filter(
                problem__id=problem["id"], accomplished=False
            ).count()
            event = ProblemActivity.objects.filter(
    ...
    favorites = EncounterEvent.objects.filter(
        encounter__patient=patient_user, is_favorite=True
    ).order_by("-datetime")

    # sharing system
    shared_patients = SharingPatient.objects.filter(sharing=patient_user).order_by(
        "shared__first_name", "shared__last_name"
    )
    patients_list = []
    ...
    sharing_patients = SharingPatient.objects.filter(shared=patient_user).order_by(
    ...
    acutes = CommonProblem.objects.filter(
        author=request.user, problem_type="acute"
    ).order_by("problem_name")
    chronics = CommonProblem.objects.filter(
        author=request.user, problem_type="chronic"
    ).order_by("problem_name")
...
def get_patients_list(request):
    ...
    is_descending = (
        True if (request.POST.get("isDescending", "true") == "true") else False
    )
    ...
    result = TopPatientSerializer(
        VWTopPatients.objects.filter(id__in=patient_ids), many=True
    ).data
...
def add_sharing_patient(request, patient_id, sharing_patient_id):
    ...
    resp["sharing_patient"] = UserProfileSerializer(
        to_sharing_patient_profile.profile
    ).data
    return ajax_response(resp)
...
def get_user_todo(request, patient_id):
    ...
    todo = ToDo.objects.filter(
        patient_id=patient_id, accomplished=is_accomplished
    ).order_by("order")
...
def get_most_recent_encounter(request, patient_id):
    ...
    encounter = (
        Encounter.objects.filter(patient_id=patient_id).order_by("-starttime").first()
    )
    ...
    if encounter:
        ...
        for event in most_recent_encounter_events:
            if (
                not "Started encounter by" in event.summary
                and not "Stopped encounter by" in event.summary
            ):
                most_recent_encounter_summaries.append(event.summary)

        # Encounter's document
        documents = ObservationValue.objects.filter(
            created_on__range=(
                encounter.starttime.replace(hour=0, minute=0, second=0, microsecond=0),
                datetime.now(),
            )
        ).filter(component__observation__subject=encounter.patient)
